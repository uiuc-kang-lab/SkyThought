...
def main_task(config, compute_score=None):
    ...
    reward_fn = RewardManager(
        tokenizer=tokenizer, num_examine=0, config=config, compute_score=compute_score
    )

    # Note that we always use function-based RM for validation
    val_reward_fn = RewardManager(
        tokenizer=tokenizer, num_examine=1, config=config, compute_score=compute_score
    )
