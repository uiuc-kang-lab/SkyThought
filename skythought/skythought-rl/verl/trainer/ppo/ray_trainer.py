...
def compute_advantage(
    data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, config=None
):
    ...
    if adv_estimator == "gae":
        ...
        advantages, returns = core_algos.compute_rloo_returns(
            data=data, eos_mask=response_mask, n_samples=num_repeat, config=config
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == "reinforce++":
        advantages, returns = core_algos.compute_reinforce_pp_returns(
...
class RayPPOTrainer(object):
    ...
    def __init__(
        ...
        if self.use_reference_policy:
            if config.algorithm.kl_ctrl.type == "fixed":
                ...
                ), f"horizon must be larger than 0. Got {config.algorithm.kl_ctrl.horizon}"
    ...
    def init_workers(self):
        ...
        elif self.config.algorithm.adv_estimator in ["grpo", "rloo", "reinforce++"]:
            print("No critic is used")
