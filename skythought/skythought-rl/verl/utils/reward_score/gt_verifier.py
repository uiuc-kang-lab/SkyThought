...
async def process_row_with_timeout(
    completion, reference, task, executor, timeout=300.0
):
    """
        ...
        tasks = [
            asyncio.wait_for(
                loop.run_in_executor(
                    executor,
                    partial(
                        process_completion, completion, task, reference
                    ),  # Ensure synchronous
                ),
                timeout=timeout,
            )
...
async def parallel_evaluate_continual_async(
    completions, references, tasks, num_processes, task_timeout=300.0
):
    """
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        tasks_async = [
            process_row_with_timeout(
                completion, reference, task, executor, timeout=task_timeout
            )
    ...
    for result, completion, reference, task in zip(
        ...
        try:
            ...
            if (
                task == "code" and not result[0][0]
            ):  # if task is code, the reference should be json string
                correct = 0
                total = min(
                    len(
                        json.loads(reference)["inputs"]
                ...
                for run in result[0][1]:
                    if (
                        "test_case" in run
                        and "res" in run["test_case"]
                        and run["test_case"]["res"] == "[True]"
                    ):
                        correct += 1
...
def compute_gt_score(completions, references, tasks):
    ...
    try:
        return asyncio.run(
            parallel_evaluate_continual_async(
                completions, references, tasks, num_processes=64
            )
        )
