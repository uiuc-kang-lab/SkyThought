...
def modify_entry(entry, idx):
    ...
    entry["generated_tests"]["output"] = list(
        entry["generated_tests"].get("output", [])
    )
    ...
    private_test_cases = []
    for inp, out in zip(
        entry["private_tests"]["input"], entry["private_tests"]["output"]
    ):
    ...
    entry["public_test_cases"] = json.dumps(
        entry.pop("public_tests")
...
for entry in filtered_lcb_codegen_list:
    extracted_private_tests[entry["question_id"]] = json.loads(
